...
public:
    ...
    struct CanceledOrder {
        Order order;
        string reason;  // e.g., "user_cancel", "market_unfilled"
        string type;    // "manual" or "automatic"
    };
    ...
    void modifyOrder(int orderId, string field, double value) {
        ...
        // First remove the old order from the book without deleting from allOrders
        bool found = false;
        ...
        // Update allOrders with modified order
        delete oldOrder;
        
        // Match the modified order
    ...
    // i want to store last assigned id in the last_id.txt file
    void saveLastAssignedId() {
        ofstream file("last_id.txt");
        file << orderIdCounter - 1; // Save the last assigned ID
    }

    int loadLastAssignedId() {
        ifstream file("last_id.txt");
        if (file.is_open()) {
            file >> orderIdCounter; // Load the last assigned ID
            file.close();
        } else {
            // File doesn't exist, create it and initialize with 0
            std::ofstream outfile("last_id.txt");
            if (outfile.is_open()) {
                outfile << 0;
                outfile.close();
            } else {
                std::cerr << "Error: Unable to create last_id.txt file." << std::endl;
            }
            return 1;
        }
        return ++orderIdCounter; // Return the next available ID
    }   
};
